...
<li>
<button class="op-card ${o.featured ? "featured" : ""}" type="button"

              aria-pressed="false" data-op="${o.id_operadora}">
<span class="op-logo op-logo--${o.nome.toLowerCase()}" aria-hidden="true">${o.nome.slice(0, 2).toUpperCase()}</span>
<span class="h3" style="font:inherit"><strong>${o.nome}</strong> ${o.featured ? '<span class="badge badge--soul">Destaque</span>' : ""}</span>
<span class="op-meta">${o.cidade} · ${o.modal.toLowerCase()} · passagem ${fmtReais(o.valor_passagem_centavos)}</span>
</button>
</li>`).join("");
  ...
  // ---- Faixas (regra de negócio: bloqueia acima do saldo) ----
...
<span class="pontos">${fmtPontos(f.pontos_necessarios)} pts</span>
            ...
            ${insuf ? '<span class="badge badge--exp" style="margin-top:8px">Saldo insuficiente</span>' : ""}
</button>
</li>`;
  ...
  if (gerarBtn) {
    ...
    gerarBtn.addEventListener("click", () => {
      ...
      const params = new URLSearchParams({ op: opSel.id_operadora, faixa: faixaSel.id_faixa });
